...
public class Player {
    ...
    public String useNonWeaponItem(BaseItem item) {
        ...
        if(item instanceof BaseArmor){
            ...
            s.add( wear(baseArmor));
        }else if(item instanceof Healable){
            Healable healable = (Healable) item;
            s.add(useHeal(healable));
        }else if(item instanceof AttBuffable){
    ...
    public void setAttBuffing(int attBuffing) {
        this.attBuffing = max(0,attBuffing);
    }
    ...
    public void setBuffRemainingTurn(int turn) {
        this.buffRemainingTurn = max(0,turn);
